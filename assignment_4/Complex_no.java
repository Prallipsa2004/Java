...
class Complex_no {
    ...
    public static Complex_no add(Complex_no complex1, Complex_no complex2) {
        return new Complex_no(complex1.real + complex2.real, complex1.imag + complex2.imag);
    }
}

class Complex_no_data {
    public static void main(String[] args) {
        Complex_no complex1 = createComplex("first");
        Complex_no complex2 = createComplex("second");

        System.out.println("The two complex numbers are: ");
        complex1.display();
        complex2.display();

        Complex_no sum = Complex_no.add(complex1, complex2);
        System.out.print("Sum of two complex numbers is: ");
        sum.display();
    }

    private static Complex_no createComplex(String name) {
        Scanner sc = new Scanner(System.in);
        System.out.print("Enter real and imag for " + name + " complex number: ");
        double real = sc.nextDouble();
        double imag = sc.nextDouble();
        return new Complex_no(real, imag);
    }
}